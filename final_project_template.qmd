...
While a separate‑milestone ITS model was initially explored to isolate individual intervention years, the grouped pandemic‑era specification was ultimately selected for combined analyses with opioid interventions, as it provided greater stability and interpretability; this choice will be consistently reflected in the code, outputs, and narrative going forward.
...
While individual intervention years were initially modeled, the grouped phase specification was ultimately chosen for opioid mortality analyses, as it reduced collinearity and provided greater stability and interpretability; this choice will be consistently reflected in the code, outputs, and narrative going forward.
...
**Combined ITS Regression for COVID and Opioid Mortality (Grouped Phases)**: In the combined interrupted time series regression, I examined COVID‑specific and opioid mortality simultaneously using grouped phases to reduce collinearity and improve interpretability. The model explained **98.4% of the variation (adjusted R² = 0.972)** with a highly significant overall fit (F ≈ 80.7, p \< 1e‑9).

For **COVID mortality**, the onset of the pandemic disruption era in 2020 produced a very large and highly significant spike of approximately **+3,153 deaths (p \< 0.001)**, followed by a steep and significant decline of about **–825 deaths per year (p \< 0.001)** across 2020–2022. The post‑COVID stabilization era (2023–2024) showed only a small, non‑significant level change (**≈ +100 deaths, p ≈ 0.52**) and a modest, non‑significant slope (**≈ –102 deaths per year, p ≈ 0.28**). These results confirm that the pandemic onset was the dominant driver of COVID mortality, with sharp declines occurring during the disruption era, while the stabilization phase reflected already low levels of deaths.

For **opioid mortality**, baseline deaths were significantly higher than COVID, with an offset of **≈ +518 deaths relative to the COVID baseline (p \< 0.001)**. The **fentanyl surge era (2014–2016)** was associated with a small negative level change (**≈ –97 deaths, p ≈ 0.38**) and a positive slope (**≈ +84 deaths per year, p ≈ 0.09**), neither statistically significant. By contrast, the **harm‑reduction era (2017–2023)** showed a large and highly significant immediate increase (**≈ +376 deaths, p \< 0.001**), while its slope effect was negligible (**≈ +15 deaths per year, p ≈ 0.27**).

Taken together, these results highlight the contrast between the sharp COVID shock and the slower opioid epidemic. COVID mortality was dominated by the dramatic onset in 2020 and subsequent steep declines, while opioid mortality rose steadily, accelerated during the fentanyl surge, and remained elevated into the harm‑reduction era despite interventions. Grouping interventions into broader phases provided stable, interpretable estimates across both crises, underscoring how COVID vaccines produced clear reductions in mortality while opioid measures yielded more gradual and variable impacts.

```{r}
# Step 1: Prepare COVID data (grouped phases)
covid_data <- covid_totals %>%
  mutate(cause = "COVID",
         annual_totals = covid_deaths) %>%
  dplyr::select(year, annual_totals, cause)

# Step 2: Prepare Opioid data (grouped phases)
...
# Step 4: Define grouped interventions
...
# Step 5: Fit combined ITS regression (grouped phases)
combined_model <- lm(
...
# Step 8: Add fitted values
combined_data <- combined_data %>%
  mutate(fitted = predict(combined_model))

# Step 9: Overlay plot (COVID + Opioid in one panel)
...
**Combined ITS Regression and Coefficient Comparison (Grouped Phases):** This combined interrupted time series (ITS) regression integrates both COVID‑specific and opioid mortality into a single model, allowing direct comparison of intervention effects across two overlapping public health crises in Philadelphia. By pooling annual deaths into one dataset with a cause indicator, the model estimates baseline differences, intervention level changes, and slope adjustments for each crisis. The regression explained **98.4% of the variation (adjusted R² = 0.972)**with a highly significant overall fit (F ≈ 80.7, p \< 1e‑9), underscoring the robustness of the grouped specification.

**COVID phases produced the largest and most consistent effects.** At the onset of the pandemic disruption era (2020–2022), deaths spiked by approximately **+3,153 (p \< 0.001)**, the single largest coefficient in the model. This was followed by a steep and highly significant downward slope of **–825 deaths per year (p \< 0.001)**, consistent with the observed decline after the initial surge. The post‑COVID stabilization era (2023+) showed only small, non‑significant changes in both level (**≈ +100 deaths, p ≈ 0.52**) and slope (**≈ –102 deaths per year, p ≈ 0.28**). Together, these results confirm that COVID onset was the dominant driver of mortality spikes, with sharp declines occurring during the disruption era, while the stabilization phase reflected already low levels of deaths.

**Opioid phases showed more mixed impacts.** Baseline opioid deaths were significantly higher than COVID, with a difference of **≈ +518 deaths (p \< 0.001)**. The fentanyl surge era (2014–2016) was associated with a small negative level change (**≈ –97 deaths, p ≈ 0.38**) and a positive slope (**≈ +84 deaths per year, p ≈ 0.09**), neither statistically significant. By contrast, the harm‑reduction era (2017–2023) showed a large and highly significant immediate increase (**≈ +376 deaths, p \< 0.001**) but no significant slope effect (**≈ +15 deaths per year, p ≈ 0.27**).

The **coefficient comparison plot** visually reinforces these findings. COVID phases (shown in blue) dominate the positive and negative shifts, with onset producing the largest spike and the disruption slope driving significant declines. Opioid phases (shown in red) cluster closer to zero, with only the harm‑reduction onset producing a statistically significant increase. Error bars highlight the uncertainty around opioid estimates, reflecting the smaller annual counts and overlapping interventions.

Taken together, the combined regression and coefficient plot highlight the contrast between crises. COVID mortality was characterized by a sudden shock and clear declines during the disruption era, while opioid mortality followed a slower trajectory with uneven intervention impacts. The results underscore that while COVID interventions produced consistent and significant reductions, opioid measures yielded mixed outcomes, with the harm‑reduction era coinciding with a significant increase in deaths rather than a decline.
...
**Interrupted Time Series Analysis of COVID and Opioid Interventions in a Combine Model:** To evaluate the impact of interventions across two overlapping public health crises, I constructed three interrupted time series (ITS) regression models: a COVID‑only model, an opioid‑only model, and a combined model that pooled both causes of death with a cause indicator. By grouping interventions into broader phases, the models avoided collinearity and produced more stable estimates.

The **COVID‑only model** isolated pandemic phases, showing a dramatic spike at the onset of the disruption era (2020–2022) of approximately **+3,240 deaths (p \< 0.001)**, followed by a steep and highly significant downward slope of **–801 deaths per year (p \< 0.001)**. The post‑COVID stabilization era (2023+) was associated with a modest positive level change (**+252 deaths, p ≈ 0.004**) but no significant slope effect (**–79 deaths per year, p ≈ 0.056**).

The **opioid‑only model** focused on overdose phases. Baseline deaths were significant (**≈ 353 deaths, p \< 0.001**) with a strong upward pre‑trend (**+35 deaths per year, p \< 0.001**). The fentanyl surge era (2014–2016) showed no significant impact (**–59 deaths, p ≈ 0.56; slope +73 per year, p ≈ 0.13**). By contrast, the harm‑reduction era (2017–2023) was associated with a large and highly significant increase (**+379 deaths, p \< 0.001**) but no meaningful slope effect (**+4 deaths per year, p ≈ 0.75**).

The **combined model** clarified which effects were robust and which were confounded. COVID pandemic disruption remained highly significant (**+3,153 deaths, p \< 0.001; slope –825 per year, p \< 0.001**), while post‑COVID stabilization effects lost significance (**+100 deaths, p ≈ 0.52; slope –102 per year, p ≈ 0.28**). This shift indicates that some of the apparent stabilization effect in the COVID‑only model was partly explained by broader mortality trends, i.e. confounding. For opioids, the fentanyl surge era remained non‑significant, but its slope effect moved closer to marginal significance (**+84 deaths per year, p ≈ 0.09**). The harm‑reduction era continued to show a consistent and significant increase (**+376 deaths, p \< 0.001**) with no slope effect.

Grouped bar charts now present a cleaner view of these coefficient shifts, with **interactive hover labels revealing confidence intervals and p‑values only when bars are explored**. This interactive design reduces visual clutter while still providing statistical rigor, showing narrow intervals for COVID effects and wide, uncertain intervals for opioid phases. The companion coefficient table reinforces these findings numerically, confirming that COVID interventions were dominant and reliable, while opioid phases produced mixed outcomes. Importantly, the comparison across models demonstrates that **confounding was present in the single‑cause specifications**: some effects appeared stronger or significant when modeled in isolation, but weakened or disappeared once both crises were included together.

Taken together, the three models demonstrate the value of triangulation. Single‑cause models provide clean baselines, the combined model reveals confounding and relative magnitudes, and interactive visualizations make stability versus instability immediately clear. Grouping interventions into phases provided a more reliable specification, underscoring the sharp COVID shock and decline versus the slower, uneven trajectory of opioid mortality.
...
**Overall Summary of ITS Analyses with Hypothesis Integration:** The Interrupted Time Series (ITS) analysis demonstrates that **COVID onset in 2020 was the dominant driver of mortality spikes in Philadelphia**, producing a sharp and highly significant increase in deaths (+3,240, p \< 0.001) followed by a steep decline during the disruption era (–801 per year, p \< 0.001). By 2023, the post‑COVID stabilization phase showed only modest and statistically weaker changes (+252 deaths, p ≈ 0.004 in the single‑cause model, but non‑significant in the combined model), underscoring that the pandemic shock and subsequent decline were the primary COVID‑related effects.

Opioid phases revealed a **slower, uneven trajectory**. The 2014–2016 fentanyl surge era produced no significant level change, though its slope effect approached marginal significance in the combined model (+84 per year, p ≈ 0.09). By contrast, the 2017–2023 harm‑reduction era was associated with a large and highly significant increase in deaths (+379 in the opioid‑only model; +376 in the combined model, both p \< 0.001) but no meaningful slope effect. This highlights that despite expanded interventions such as Narcan distribution, vending machines, and OTC access, overdose mortality continued to rise sharply during this period.

The **combined model clarified confounding** between crises. Effects that appeared significant in single‑cause models (such as the COVID post‑stabilization level change) weakened or disappeared once both causes were modeled together. Similarly, opioid slope effects shifted toward marginal significance when COVID mortality was included, revealing that overlapping crises can distort single‑cause estimates. Grouped bar charts and coefficient tables reinforced these findings, showing narrow confidence intervals for COVID effects and wide, uncertain intervals for opioid phases.
...
-   **H1: Crises produce sharp disruptions in mortality trends.** ✔ Confirmed — COVID onset produced the sharpest and most reliable shock; opioids rose persistently but unevenly.

-   **H2: Interventions reduce mortality.** ✔ Partially confirmed — COVID vaccines and boosters produced consistent declines; opioid harm‑reduction measures coincided with increases, showing limited effectiveness.

-   **H3: Grouping interventions into phases improves stability.** ✔ Confirmed — grouped models reduced collinearity, stabilized coefficients, and explained 97–99% of variation.

-   **H4: Combined modeling reveals confounding.** ✔ Confirmed — effects that appeared significant in single‑cause models weakened or disappeared once both crises were modeled together.
...
**Why Filtering Matters for Demographic Analysis:** By restricting to raw death counts and removing invalid or suppressed entries, the dataset reflects true mortality patterns without distortion. This ensures that subsequent demographic summaries — whether by sex, race/ethnicity, or age — are grounded in accurate counts. Without this filtering, disparities could be exaggerated or obscured, undermining the validity of the analysis. In effect, filtering is the quality‑control step that guarantees the demographic analysis is built on a reliable foundation.
...
-   **Overall mortality rates increased after the onset of COVID‑19 (2020):** ✔ **Supported.** COVID onset was consistently associated with a sharp increase in mortality (\~25–29% higher deaths).

-   **Opioid‑related mortality rose sharply after fentanyl emergence (2014):** ✔ **Supported.** Mortality counts showed strong increases following fentanyl surge, confirming the disruption.

-   **COVID‑19 deaths disproportionately affected older age groups:** ✔ **Supported.** Age was the strongest predictor, with 65+ showing \~9× higher deaths and “All ages” \~14× higher compared to 25–44.

-   **Opioid‑related deaths disproportionately affected males:** ✔ **Supported.** Male baseline consistently showed higher mortality; females were protective across models.

-   **Black and Hispanic populations experienced higher excess mortality during COVID‑19 than White populations:** ✖ **Partially supported.** Black (NH) populations consistently showed excess mortality, but Hispanic groups showed protective effects overall. COVID‑specific interactions revealed disparities, but not uniformly in the expected direction.
